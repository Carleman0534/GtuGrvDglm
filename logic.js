...
function loadFromLocalStorage() {
    ...
    if (saved) {
        ...
        if (!loadedDB.version || loadedDB.version < 36) {
            let dbToSave = loadedDB;
            if (loadedDB.version < 29) dbToSave = migrateToV29(dbToSave);
            ...
            if (dbToSave.requests && dbToSave.requests.length > 0) {
                dbToSave.requests.forEach(function(req) {
                    if (req.examName && req.examName !== 'Bilinmeyen Sınav') return; // Zaten doğru doldurulmuş
                    
                    var exams = dbToSave.exams || [];
                    var exam = null;
                    
                    // Strateji 1: examId ile doğrudan eşleştir
                    exam = exams.find(function(e) { return e.id == req.examId; });
                    
                    // Strateji 2: Devreden kişinin (initiator) atandığı sınavları bul
                    if (!exam && req.initiatorId) {
                        ...
                        // Eğer sadece 1 sınav varsa direkt eşleştir
                        if (initiatorExams.length === 1) {
                            exam = initiatorExams[0];
                        } else if (initiatorExams.length > 1) {
                            // Birden fazla varsa talebin oluşturulma zamanına en yakın olanı al
                            exam = initiatorExams[0]; // İlk sınavı varsayılan yap
                        }
                    }
                    ...
                    if (exam) {
                        ...
                        req.examId = exam.id; // ID'yi de düzelt
                    } else {
        ...
        if (v29) {
            ...
            DB = migrateToV30(migrateToV29(migrateToV28(JSON.parse(v27))));
        } else {
            seedInitialExams();
            return;
        }
...
// ── KRİTİK GÜVENLE MIGRATE ──────────────────────────────────────────────
    // Bazı eski kayıtlar proctorId/proctorName formatında olabilir.
    // Bunları proctors: [{id, name}] formatına çevir.
    if (DB && DB.exams) {
        DB.exams.forEach(ex => {
