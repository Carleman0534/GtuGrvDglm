...
function initUI() {
    ...
    if (btnConfirm) {
        btnConfirm.addEventListener('click', () => {
            ...
            // --- Yardımcı: Farklı tarih formatlarını YYYY-MM-DD'ye çevir ---
            ...
            // --- Yardımcı: Saatten sadece HH:MM al ---
            ...
            // --- Yardımcı: Süre alanından sayıyı çıkar ("120 dk" → 120) ---
...
window.showStaffSchedule = (staffName) => {
        ...
        tr.innerHTML = `
            <td>
                <div style="display:flex; align-items:center; gap:8px;">
                    <button class="btn-calendar" onclick="window.open(generateGoogleCalendarLink(${JSON.stringify(ex).replace(/"/g, '&quot;')}), '_blank')" title="Google Takvim'e Ekle" style="padding: 2px 5px; font-size: 0.8rem;">📅</button>
    ...
    // PDF Butonuna olay dinleyicisi ekle (Eğer yoksa)
    const btnPdf = document.getElementById('btn-export-individual-schedule-pdf');
    if (btnPdf) {
        // Eski listener'ları temizlemek için klonlama yöntemi (Opsiyonel ama güvenli)
...
function renderMyProfile() {
    ...
    if (profileIcsBtn) profileIcsBtn.style.display = 'block';
    ...
    sortedToDisplay.forEach(ex => {
        ...
        tr.innerHTML = `
            <td><span class="badge-type ${typeClass}" style="zoom: 0.8;">${typeLabel}</span> <strong>${ex.name}</strong></td>
