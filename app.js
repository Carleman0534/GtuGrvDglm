    ...
    matchingExams.forEach(ex => {
        ...
        pIds.forEach(pid => {
            ...
            if(st) {
                 ...
                 st.totalScore = parseFloat(Math.max(0, st.totalScore - (duration * 1.5)).toFixed(2));
            }
        });
    });
    ...
    for(let i=0; i<requiredProctors; i++) {
        ...
        if(best && best.id) {
            ...
            if(st) {
                 ...
                 st.totalScore = parseFloat((st.totalScore + (duration * 1.5)).toFixed(2));
            }
    ...
    selectedCourses.forEach((courseName, courseIdx) => {
        ...
        for (let w = 0; w < weekCount; w++) {
            ...
            DB.exams.push({
                ...
                location: prefLoc,
                lecturer: prefLect,
                score, katsayi: getKatsayi(new Date(`${dateStr}T${prefTime}`)),
                proctorIds: [], proctorId: null, proctorName: '',
